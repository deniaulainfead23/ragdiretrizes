...
EVIDENCE_CSV = ROOT / "dados_derivados" / "analysis" / "consolidado" / "evidencias_consolidadas.csv"
RESPONSES_CSV = ROOT / "dados_derivados" / "analysis" / "consolidado" / "respostas_consolidadas.csv"
FRAMEWORK_CSV = ROOT / "rag_project" / "framework" / "unesco_dlgf_2018.csv"
OUTPUT_DIR = ROOT / "dados_derivados" / "analysis" / "dlgf_2018"
...
def main() -> None:
    ...
    for row in evidence:
        ...
        if not original_text:
            original_text = row.get("source_text", "").strip()
        ...
        for match in matches:
            output_rows.append({
                ...
                "document": row.get("document", "") or row.get("document_title", ""),
                "page": row.get("page", "") or row.get("page_start", ""),
                "original_text": original_text,
                "classification_original": row.get("classification", ""),
                "validation_status_original": row.get("evidence_validation_status", "") or row.get("validation_status", ""),
